...
<List>
  ...
  const fetchData =  async () => {
    try{
     setDisplayValue(4)
      const response = await axios.get(`${config.url}/getallhoteladmins`)
if(response.data === ""){
  navigate('/login')
}else if(response.data.length  === 0) {
  setMessage("No hotelmanagers in your portel");
  setDisplayValue(5)
...
useEffect(()=>{
  ...
  const viewHotelAdmindetails = async(hoteladmin) =>{
    try{
    setOpen(true)
    setBoxValue(0)
   setHotelAdmin(hoteladmin)
   setBoxValue(1)
  }catch(error){
    setMessage(error.message)
    setDisplayValue(5)      
  ...
  const Accepect = async(hoteladmin) =>{
    if(hoteladmin.status === 1){
        alert("Hotelmanager exists in your portel")
        return
    }
    try{
      setDisplayValue(4)
      const response1 = await axios.post(`${config.url}/hoteladminapprovel`,hoteladmin);
      if(response1.data === ""){
        navigate('/login')
        return
      }
  ...
  const DeleteAccount = async(hoteladmin) =>{
    try{
        setDisplayValue(4)
        const response1 = await axios.post(`${config.url}/deletehoteladminbyid`,hoteladmin);
        if(response1.data === ""){
          navigate('/login')
          return
        }
        else if (response1.data === 1){
          alert("Account Account deleated")
          setDisplayValue(3)
          window.location.reload()
          return
        }
...
<Box sx={{ overflowX: 'auto' }}>
      ...
      aria-label="responsive table">
        <TableHead>
          <TableRow>            
 <TableCell align="left" sx={{fontWeight: 'bold',backgroundColor: '#673ab7',color: '#fff',fontSize: { xs: '12px', sm: '14px', md: '16px' },padding: { xs: '8px', sm: '10px' },borderBottom: '2px solid #fff',}}>Hotelmanager id</TableCell>
 <TableCell align="left" sx={{fontWeight: 'bold',backgroundColor: '#673ab7',color: '#fff',fontSize: { xs: '12px', sm: '14px', md: '16px' },padding: { xs: '8px', sm: '10px' },borderBottom: '2px solid #fff',}}>Hotelmanager email</TableCell>          
 <TableCell align="left" sx={{fontWeight: 'bold',backgroundColor: '#673ab7',color: '#fff',fontSize: { xs: '12px', sm: '14px', md: '16px' },padding: { xs: '8px', sm: '10px' },borderBottom: '2px solid #fff',}}>Hotelmanager details</TableCell>          
 <TableCell align="left" sx={{fontWeight: 'bold',backgroundColor: '#673ab7',color: '#fff',fontSize: { xs: '12px', sm: '14px', md: '16px' },padding: { xs: '8px', sm: '10px' },borderBottom: '2px solid #fff',}}>Accepect</TableCell>
        ...
        <TableBody>
          {data.map((data,index) => (
            ...
            >
              ...
              <TableCell sx={{ padding: { xs: '8px', sm: '10px' } }}><Button onClick={() => {viewHotelAdmindetails(data)}} variant="contained" color="secondary" sx={{fontSize: { xs: '10px', sm: '12px' },padding: { xs: '5px', sm: '8px' },textTransform: 'none',}}>View</Button></TableCell>
              <TableCell sx={{ padding: { xs: '8px', sm: '10px' } }}><Button onClick={() => {Accepect(data)}} variant="contained" color="secondary" sx={{fontSize: { xs: '10px', sm: '12px' },padding: { xs: '5px', sm: '8px' },textTransform: 'none',}}>Accepect</Button></TableCell> 
 ...
 <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: 4 }}>
        ...
        >
          ...
          <CardMedia
            ...
            alt="User Profile"
            height="200"
            image={HotelAdmin.hotelimageinbytes} // Fallback image
            sx={{ objectFit: 'cover' }}
          />
          <CardContent>
            <Typography
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <Typography variant="body1" color="text.secondary">
                  <strong>Hotelmanager name:</strong> {HotelAdmin.name}
                </Typography>
              </Grid>
              </Grid>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Typography variant="body1" color="text.secondary">
                  <strong>Sex:</strong> {HotelAdmin.sex}
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography variant="body1" color="text.secondary">
