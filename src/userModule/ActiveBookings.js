...
<List>
  ...
  const fetchData =  async () => {
    try{
      setDisplayValue(2)
      const response = await axios.get(`${config.url}/roombookingsbasedonuserid`)
...
useEffect(()=>{
  ...
  const Hoteldatails = async(hoteladminid) =>{
    try{
     setOpen(true)
    setBoxValue(0)
     const response1 = await axios.post(`${config.url}/gethoteladminbyid`,{
        id:hoteladminid
     });
     if(response1.data === ""){
      navigate('/login')
  ...
  const Roomdetails = async(roomid) =>{
    try{
      setOpen(true)
      setBoxValue(0)
        const response1 = await axios.post(`${config.url}/getroombyid`,{
            sno:roomid
         });
         if(response1.data === ""){
          navigate('/login')
  ...
  const cancel = async(id) =>{
    try{
      setDisplayValue(2)
      const response1 = await axios.post(`${config.url}/cancelroomschedule`,{
        orderid:id
     });
    if(response1.data === "1"){
      alert("Your Booking deleated successfully")
    }
 ...
 <Box sx={{ overflowX: 'auto' }}>
  ...
  <TableCell align="left" sx={{fontWeight: 'bold',backgroundColor: '#673ab7',color: '#fff',fontSize: { xs: '12px', sm: '14px', md: '16px' },padding: { xs: '8px', sm: '10px' },borderBottom: '2px solid #fff',}}>Hotel tetails</TableCell>
  ...
  <TableCell align="left" sx={{fontWeight: 'bold',backgroundColor: '#673ab7',color: '#fff',fontSize: { xs: '12px', sm: '14px', md: '16px' },padding: { xs: '8px', sm: '10px' },borderBottom: '2px solid #fff',}}>Cancel Booking</TableCell>
           </TableRow>
         </TableHead> 
         <TableBody>
           {data.map((data,index) => (
             ...
             >
               ...
               <TableCell sx={{ padding: { xs: '8px', sm: '10px' } }}><Button onClick={() => {Hoteldatails(data.hoteladminid)}} variant="contained" color="secondary" sx={{fontSize: { xs: '10px', sm: '12px' },padding: { xs: '5px', sm: '8px' },textTransform: 'none',}}>View</Button></TableCell>
               ...
               <TableCell sx={{ padding: { xs: '8px', sm: '10px' } }}><Button onClick={() => {cancel(data.orderid)}} variant="contained" color="secondary" sx={{fontSize: { xs: '10px', sm: '12px' },padding: { xs: '5px', sm: '8px' },textTransform: 'none',}}>Cancel</Button></TableCell>
             </TableRow>
           ))}
         </TableBody>
       </Table>
